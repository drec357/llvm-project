...
private:
  ...
  class VarDeclBitfields {
    ...
    // FIXME these two could perhaps be generalized to cover normal
    // for-range range vars and loop vars, not just those generated by
    // CXXExpansionStmts; that would give their place here more validity:

    /// Whether this is an implicitly generated variable named "__range",
    /// initialized by the written expression in a for statement.
    unsigned IsImplicitExpansionRangeVar : 1;

    /// Whether this is an implicitly generated loop variable, initialized
    /// as an expression of the implicit __range variable in the body of
    /// a for statement.
    unsigned IsImplicitExpansionLoopVar : 1;

    /// Whether this variable is an ARC pseudo-__strong variable; see
    /// isARCPseudoStrong() for details.
    unsigned ARCPseudoStrong : 1;
  };
  enum { NumVarDeclBits = 10 };
...
public:
  ...
  /// [TemplateFor]
  /// Whether this is an implicitly generated variable named
  /// __range, used in compile-time expansions: the thing
  // initialized to `tup` in
  /// \code
  ///   template for (constexpr auto x : tup) {...}
  /// \endcode
  bool isImplicitExpansionRangeVar() const {
    return VarDeclBits.IsImplicitExpansionRangeVar;
  }
  void setIsImplicitExpansionRangeVar(bool V = true) {
    VarDeclBits.IsImplicitExpansionRangeVar = V;
  }

  /// Whether this is a loop variable used in compile-time expansions:
  /// the `x`, initialized as a dependent expression involving `range`, in
  /// \code
  ///   template for (constexpr auto x : tup) {...}
  /// \endcode
  bool isImplicitExpansionLoopVar() const {
    return VarDeclBits.IsImplicitExpansionLoopVar;
  }
  void setIsImplicitExpansionLoopVar(bool V = true) {
    VarDeclBits.IsImplicitExpansionLoopVar = V;
  }
