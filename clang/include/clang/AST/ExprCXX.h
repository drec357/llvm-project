...
/// Represents a \c __select(I,obj) or \c __select(I,pack) expression,
/// which given a constant integer N and a class object or pack substitutes a
/// single LValue expression representing the Nth public nonstatic field of that
...
protected:
  ...
  void setSubstituteExpr(Expr *V) { Substitute = V; }

public:
  /// Whether this was implicitly generated by e.g. a template for
  /// statement, or if __select was written in the code.
  bool isImplicit() const { return getSelectLoc().isInvalid(); }

  /// The unexpanded pack or reference to a class object.
  Expr *getRangeExpr() const {
    return reinterpret_cast<Expr *>(RangeAndIndex[0]);
  }

  /// The index expression. Will either be dependent typed or integer-typed,
  /// and is always an IntegerLiteral when \c this->isImplicit() .
  Expr *getIndexExpr() const {
    return reinterpret_cast<Expr *>(RangeAndIndex[1]);
  }
  void setIndexExpr(Expr *V) { RangeAndIndex[1] = V; }

  /// The MemberExpr or DeclRefExpr to substitute for this expression during
  /// evaluation.  Null when this is a dependent expression.
  Expr *getSubstituteExpr() const { return Substitute; }
  ...
  SourceLocation getEndLoc() const LLVM_READONLY {
    // Hack: assumes right parens is one character past the end of the index.
    // We should probably store the right parens loc, but given that this
    // is mostly an implicitly generated node with no meaningful locs,
    // this is fine.
    return getIndexExpr()->getEndLoc().isValid()
        ? getIndexExpr()->getEndLoc().getLocWithOffset(1) : getBeginLoc();
  }
  ...
  child_range children() {
    return child_range(&RangeAndIndex[0], &RangeAndIndex[0] + 2);
  }
};
...
public:
  ...
  /// The range expression may either be a FunctionParmPackExpr or a
  /// SubstNonTypeTemplateParmPackExpr.  This dyn_casts to the the latter.
