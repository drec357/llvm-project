...
void f(int b) {
  if (b) {
    consteval {
      ...
      __injf("static const int {} = {} + {} + {};", "fj", 4, five, "i");
    }
    v += 0; // Referencing v here is only possible because the metaprogram was
            // non-dependent (i.e. not templated), and so was evaluated immediately
    static_assert(fi==3);
    static_assert(fj==12);
  }
}
...
// Macros: these shouldn't be used within injection statements;
